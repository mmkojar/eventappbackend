...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column .', users.id, password, active, last_login, users_groups.group_id')
...
// 		print_r($this->db->queries);
	...
	{
		$this->trigger_events('get_user_group');
		
		// if no id was passed use the current users id
		if(empty($id)){
			$id || $id = $this->session->userdata('user_id');			
		}
		$query = $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, ')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['users_groups']);
